    ...
    def run(
        ...
        if result is None:
            dispatcher.utter_message(
                text="Requested direction not recorded in the system."
            )
        else:
            message = f"Sure! To go to {result[1].title()}, you need to {result[2]}"
...
class ActionAnswerExplain(Action):
    ...
    async def run(
        ...
        explain_subject: str = next(
            tracker.get_latest_entity_values("explain_subject"), None
        )
        ...
        sentences = re.split(r"(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?)\s", result[2])
        for s in sentences:
            dispatcher.utter_message(s)
...
class ActionAnswerInquiry(Action):
    ...
    async def run(
        ...
        inquiry_subject: str = next(
            tracker.get_latest_entity_values("inquiry_subject"), None
        )
